//! `G_MATRIX` M=2 — coupled-diffusion convergence slope (`RELEASE_BLOCKING`).
//!
...
/// Generate M×M SPD matrix: A = L·Lᵀ + ε·I from random lower triangular L.
fn gen_spd<const M: usize>(rng: &mut Pcg64, eps: f64) -> [[f64; M]; M] {
    let mut l = [[0.0_f64; M]; M];
    for i in 0..M {
        for j in 0..=i {
            l[i][j] = rng.next_f64() * 0.8 + 0.1; // in (0.1, 0.9)
        }
    }
    // A = L·Lᵀ + eps·I.
...
/// Generate M×M skew-symmetric matrix (random, small magnitude).
...
/// Generate M×M symmetric matrix (for reaction).
fn gen_sym<const M: usize>(rng: &mut Pcg64) -> [[f64; M]; M] {
    let mut c = [[0.0_f64; M]; M];
    for i in 0..M {
        for j in i..M {
            let v = (rng.next_f64() - 0.5) * 0.1; // small negative ok for reaction
            c[i][j] = v;
            c[j][i] = v;
        }
    }
...
fn make_kernel(n: usize) -> MatrixDiffusionChernoff<f64, M> {
    ...
    // Pre-generate fixed matrices (not spatially varying in this gate for reproducibility).
...
fn run_sweep(n_steps: u32, reference: &MatrixGridFn1D<f64, M>) -> f64 {
    ...
    // max over components of sup-norm error.
    let n = N_GRID;
    let mut err = 0.0_f64;
    for k in 0..n {
        for i in 0..M {
...
#[test]
fn g_matrix_m2_slope() {
    // Reference at N_REF = 2048 steps.
    ...
    for (&n, &e) in ns.iter().zip(errs.iter()) {
        println!("G_MATRIX M=2: n={n} err={e:.4e}");
    }

    let slope = ols_slope(&ns, &errs);
    println!("G_MATRIX M=2: OLS slope = {slope:.4}");
    assert!(
        slope <= -1.95,
        "G_MATRIX M=2: slope {slope:.4} > -1.95 (gate FAILED)"
    );
}
