//! `G_MATRIX` M=4 — coupled-diffusion convergence slope (`RELEASE_BLOCKING`).
//!
//! Gate: slope ≤ -1.95 on n ∈ {512, 1024, 2048, 4096} at T = 0.5.
//! Reference: self-convergence against `n_ref` = 16384 (math §33.4, ADR-0082).
...
fn make_kernel(n: usize) -> MatrixDiffusionChernoff<f64, M> {
    ...
    // Skip M=2 and M=3 draws to get M=4 specific matrices.
    for _ in 0..12 {
        rng.next_f64();
    }
...
fn g_matrix_m4_slope() {
    ...
    // Sweep (stable regime: n ≥ 512 for M=4, see stability note in file header).
    let ns = [512_u32, 1024, 2048, 4096];
    let errs: Vec<f64> = ns.iter().map(|&n| run_sweep(n, &reference)).collect();

    for (&n, &e) in ns.iter().zip(errs.iter()) {
        println!("G_MATRIX M=4: n={n} err={e:.4e}");
    }

    let slope = ols_slope(&ns, &errs);
    println!("G_MATRIX M=4: OLS slope = {slope:.4}");
    assert!(
        slope <= -1.95,
        "G_MATRIX M=4: slope {slope:.4} > -1.95 (gate FAILED)"
    );
}
