...
//! # Run
//! ```bash
//! cargo test -p semiflow --features slow-tests g_s3_varcoef_spectral -- --nocapture
//! ```
...
fn r_matvec(u: &[f64], a: &[f64], b: &[f64], n: usize, dx: f64, a0: f64) -> Vec<f64> {
    ...
    for i in 0..n {
        ...
        let a_ip = 0.5 * (a[i] + a[ip]);
        let a_im = 0.5 * (a[i] + a[im]);
        let lju = a_ip * (u[ip] - u[i]) / dx2 - a_im * (u[i] - u[im]) / dx2
            + b[i] * (u[ip] - u[im]) / two_dx;
...
fn g_s3_varcoef_spectral() {
    ...
    {
        let src_path = concat!(env!("CARGO_MANIFEST_DIR"), "/src/tt_varcoef_spectral.rs");
        let src = std::fs::read_to_string(src_path).expect("cannot read tt_varcoef_spectral.rs");
    ...
    // ── Assert 2: LAYER-1 EXACTNESS ─────────────────────────────────────
    {
        ...
        let l_lifted: Vec<Vec<f64>> = (0..d)
            .map(|j| lift_axis(&gen_1d(&coef_a(&xs, j), &coef_b(&xs, j), n, dx), n, d, j))
            .collect();
    ...
    // Verify rank=1 by: σ₁ > 0 AND ‖M - σ₁ u₁ v₁^T‖_F / ‖M‖_F < 1e-12.
    {
        ...
        for _ in 0..40 {
            ...
            let v_tmp: Vec<f64> = (0..cols)
                .map(|j| (0..rows).map(|i| m_mat[i * cols + j] * u[i]).sum())
                .collect();
            // u_new = M v_tmp (rows-dim)
            let mut u_new: Vec<f64> = (0..rows)
                .map(|i| (0..cols).map(|j| m_mat[i * cols + j] * v_tmp[j]).sum())
                .collect();
        ...
        let res_sq: f64 = (0..rows)
            ...
            .sum();
        let rel_res = if frob > 1e-300 {
            res_sq.sqrt() / frob
        } else {
            0.0
        };
        println!(
            "Assert 3: σ₁={sigma1:.6e}, ‖M-σ₁u₁v₁ᵀ‖_F/‖M‖_F={rel_res:.3e} \
    ...
    // ── Assert 1: ORDER-2 CONVERGENCE ───────────────────────────────────
    {
        ...
        let tot = n.pow(d as u32);
        let l_full: Vec<f64> = (0..d)
            .map(|j| lift_axis(&gen_1d(&a_axis[j], &b_axis[j], n, dx), n, d, j))
        ...
        println!(
            "  log-log slope d(log err)/d(log tau) = {slope:.4}  (target ≥ 1.9, probe=2.0000)"
        );
        assert!(
            slope >= 1.9,
    ...
    // ── Assert 4: BOUNDARY — non-separable cross-diffusion ──────────────
    {
        ...
        let tot = n.pow(d as u32);
        let mut l_true: Vec<f64> = (0..d)
            .map(|j| lift_axis(&gen_1d(&a_axis[j], &b_zero[j], n, dx), n, d, j))
            .fold(vec![0.0f64; tot * tot], |acc, lj| mat_add(&acc, &lj));
        ...
        println!(
            "  slope={slope:.4}  floor={floor:.3e}  (target: slope<1.0, floor>1e-4; probe slope≈0)"
        );
        assert!(
            slope < 1.0,
        ...
        println!(
            "Assert 4 PASS: slope={slope:.4} < 1.0 AND floor={floor:.3e} > 1e-4 \
    ...
    // ── Assert 5: LOAD-BEARING variable coefficient ──────────────────────
    {
        ...
        let a_const: Vec<Vec<f64>> = a_axis
            .iter()
            .map(|aj| {
        ...
        println!(
            "Assert 5 PASS: variable coefficient is load-bearing \
    ...
    // ── Assert 6: COST-SCALING — d=8,10 finite+real + TB check ─────────
    {
        ...
        for &d in &[8usize, 10usize] {
            ...
            assert!(all_finite, "assert 6 FAIL: evolver non-finite at d={d}");
            assert!(
                dense_bytes > CURSE_TB,
