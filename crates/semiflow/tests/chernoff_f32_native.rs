...
use semiflow::{
    chernoff::ApplyChernoffExt, diffusion::DiffusionChernoff, diffusion4::Diffusion4thChernoff,
    diffusion6::Diffusion6thChernoff, drift_reaction::DriftReactionChernoff,
    shift1d::ShiftChernoff1D, truncated_exp::TruncatedExpDiffusionChernoff,
    truncated_exp4::TruncatedExp4thDiffusionChernoff, Grid1D, GridFn1D,
};
...
fn diffusion_chernoff_f32_native() {
    let g = grid_f32();
    let k = DiffusionChernoff::<f32>::new(|_| 0.5_f32, |_| 0.0_f32, |_| 0.0_f32, A_NORM, g);
    let f = gaussian_f32(g);
    let out = k
        .apply_chernoff(TAU, &f)
        .expect("DiffusionChernoff<f32> apply");
...
fn diffusion4_chernoff_f32_native() {
    let g = grid_f32();
    let k =
        Diffusion4thChernoff::<f32>::new_generic(|_| 0.5_f32, |_| 0.0_f32, |_| 0.0_f32, A_NORM, g);
    let f = gaussian_f32(g);
    let out = k
        .apply_chernoff(TAU, &f)
        .expect("Diffusion4thChernoff<f32> apply");
    assert_eq!(out.values.len(), N, "output length mismatch");
...
fn diffusion6_chernoff_f32_native() {
    let g = grid_f32();
    let k =
        Diffusion6thChernoff::<f32>::new_generic(|_| 0.5_f32, |_| 0.0_f32, |_| 0.0_f32, A_NORM, g);
    let f = gaussian_f32(g);
    let out = k
        .apply_chernoff(TAU, &f)
        .expect("Diffusion6thChernoff<f32> apply");
    assert_eq!(out.values.len(), N, "output length mismatch");
...
fn drift_reaction_chernoff_f32_native() {
    let g = grid_f32();
    let k = DriftReactionChernoff::<f32>::new_generic(|_| 0.1_f32, |_| 0.0_f32, 0.0, g);
    let f = gaussian_f32(g);
    let out = k
        .apply_chernoff(TAU, &f)
        .expect("DriftReactionChernoff<f32> apply");
    assert_eq!(out.values.len(), N, "output length mismatch");
...
fn truncated_exp_chernoff_f32_native() {
    ...
    let out = k
        .apply_chernoff(TAU, &f)
        .expect("TruncatedExpDiffusionChernoff<f32> apply");
    assert_eq!(out.values.len(), N, "output length mismatch");
...
fn truncated_exp4_chernoff_f32_native() {
    ...
    let out = k
        .apply_chernoff(TAU, &f)
        .expect("TruncatedExp4thDiffusionChernoff<f32> apply");
    assert_eq!(out.values.len(), N, "output length mismatch");
...
fn shift_chernoff1d_f32_native() {
    let g = grid_f32();
    let k = ShiftChernoff1D::<f32>::new_generic(|_| 0.5_f32, |_| 0.1_f32, |_| 0.0_f32, 0.0, g);
    let f = gaussian_f32(g);
    let out = k
        .apply_chernoff(TAU, &f)
        .expect("ShiftChernoff1D<f32> apply");
    assert_eq!(out.values.len(), N, "output length mismatch");
