    ...
    for axis in 0..d {
        ...
        for i_outer in 0..n_outer {
            for i_inner in 0..stride {
                for k in 0..n {
                    line[k] = u_w[i_outer * n * stride + k * stride + i_inner];
                }
                ...
                for k in 0..n {
                    u_w[i_outer * n * stride + k * stride + i_inner] = iv[2 * k];
                }
            }
        }
    }
...
fn nonsep_evolve_local(
    ...
    let mut u = u0.to_vec();
    for _ in 0..nsteps {
        u = nonsep_step_local(&u, r, m, n, d, dx, a0, tau);
    }
    u
}
...
fn build_true_2d(n: usize, dx: f64, role: &str) -> (Vec<f64>, Vec<f64>) {
    ...
    for i in 0..nd {
        for j in 0..nd {
            l[i * nd + j] =
                A0 * lx[i * nd + j] + A0 * ly[i * nd + j] + a_diag[i] * core[i * nd + j];
        }
    }
    // R = L - a0*(Lx+Ly)
    let r: Vec<f64> = (0..nd * nd)
        .map(|i| l[i] - A0 * lx[i] - A0 * ly[i])
        .collect();
    (l, r)
}
...
fn log_slope(taus: &[f64], errs: &[f64], drop: usize) -> f64 {
    ...
    let ys: Vec<f64> = errs[drop..]
        .iter()
        .map(|&e| if e <= 0.0 { f64::NEG_INFINITY } else { e.ln() })
        .collect();
    let m = xs.len() as f64;
...
fn matrix_rank_power(a: &[f64], rows: usize, cols: usize, eps: f64) -> usize {
    ...
    for _iter in 0..rows.min(cols).min(20) {
        ...
        let mut v: Vec<f64> = (0..cols)
            .map(|i| ((i as f64 + 1.0) * 0.31 + 0.7).sin())
            .collect();
        ...
        for _ in 0..60 {
            ...
            let u: Vec<f64> = (0..rows)
                .map(|i| {
                    (0..cols)
                        .map(|j| deflated[i * cols + j] * v[j])
                        .sum::<f64>()
                })
                .collect();
            // v_new = A^T u
            let mut v_new: Vec<f64> = (0..cols)
                .map(|j| {
                    (0..rows)
                        .map(|i| deflated[i * cols + j] * u[i])
                        .sum::<f64>()
                })
                .collect();
        ...
        if sigma < eps * first_sv.max(1e-300) {
            break;
        }
        rank += 1;
...
fn build_residual_cp(n: usize, d: usize, dx: f64, cp_rank: usize, amp: f64) -> Vec<f64> {
    ...
    for i in 0..nd {
        for j in 0..nd {
            r_mat[i * nd + j] = a_vals[i] * core[i * nd + j];
        }
    }
    r_mat
}
...
fn g_s3_nonsep_varcoef() {
    ...
    // Evolver: INDEPENDENT local reimplementation (zero reuse of production module).
    {
        ...
        assert!(
            errs[0] > 1e-7,
            "Assert 1 FAIL: coarsest err {:.3e} not in real regime (need >1e-7)",
            errs[0]
        );
        assert!(
            floor < 1e-7,
        ...
        println!(
            "Assert 1 PASS: cos(x)sin(y) diffusion CONVERGES slope={slope:.4} floor={floor:.3e}"
        );
    }
    ...
    // ── Assert 2: ALL-ROLES ORDER-2 ──────────────────────────────────────────
    {
        ...
        for role in ["diffusion", "drift", "potential"] {
            ...
            assert!(
                slope >= 1.9,
                "Assert 2 FAIL ({role}): slope {slope:.4} < 1.9"
            );
        }
    ...
    // FULL-R must converge; MEAN-FROZEN must floor (reproduces 0166 boundary ~9.548e-3).
    {
        ...
        for &ns in &nsteps_list {
            ...
            taus.push(tau);
            println!(
                "  ns={ns:4} FULL={:.4e} ABLATE={:.4e}",
                errs_full.last().unwrap(),
                errs_ablate.last().unwrap()
            );
        }
        ...
        println!(
            "  Probe: FULL slope +2.0000 floor ~2.58e-10; MEAN-FROZEN slope ~0 floor 9.548e-3"
        );

        assert!(
            slope_full >= 1.9,
            "Assert 3 FAIL: FULL-R slope {slope_full:.4} < 1.9"
        );
        assert!(
            floor_full < 1e-4,
            "Assert 3 FAIL: FULL-R floor {floor_full:.3e} ≥ 1e-4"
        );
        assert!(
            slope_ablate > -1.0,
            "Assert 3 FAIL: MEAN-FROZEN slope {slope_ablate:.4} not > -1.0"
        );
        assert!(
            floor_ablate > 1e-4,
            "Assert 3 FAIL: MEAN-FROZEN floor {floor_ablate:.3e} not > 1e-4"
        );
    ...
    // ── Assert 4: OPERATOR-TT-RANK = CP-RANK (max-over-ALL-bonds) ──────────
    {
        ...
        for (label, cp_rank) in [("CP-rank-1", 1usize), ("CP-rank-2", 2), ("CP-rank-3", 3)] {
            ...
            for &rk in &ranks_all {
                assert_eq!(
                    rk, cp_rank,
                    "Assert 4 FAIL: {label} max rank {rk} ≠ {cp_rank}"
                );
            }
        }
        ...
        println!(
            "  GENERIC:       d=2,3,4 -> axis0|rest rank {ranks_gen:?} (expect [{n},{n},{n}])"
        );
        for &rk in &ranks_gen {
            assert_eq!(rk, n, "Assert 4 FAIL: generic axis0 rank {rk} ≠ n={n}");
        }
    ...
    // ── Assert 5: NEGATIVE BOUNDARY (generic op-rank = n) ──────────────────
    {
        ...
        for d in [2usize, 3, 4] {
            ...
            assert_eq!(
                rk, n,
                "Assert 5 FAIL: generic axis0 rank {rk} ≠ n={n} at d={d}"
            );
        }
    ...
    // NOTE: this assert verifies the production module `nonsep_evolve` runs at d=8,10.
    {
        ...
        for d in [8usize, 10] {
            ...
            assert!(
                bytes > CURSE_TB,
                "Assert 6 FAIL: n^{{2d}}*8={bytes:.3e} ≤ {CURSE_TB:.1e}"
            );
            ...
            let (sub, main_d, sup) = (
                vec![1.0 / ((TAU / n as f64).powi(2)); n],
                vec![-2.0 / ((TAU / n as f64).powi(2)); n],
                vec![1.0 / ((TAU / n as f64).powi(2)); n],
            );
            ...
            let apply_r_rank1 = |u: &[f64]| -> Vec<f64> {
                ...
                for line_flat in 0..stride0 {
                    ...
                    let mut line = vec![0.0f64; n];
                    for i0 in 0..n {
                        line[i0] = u[i0 * stride0 + line_flat];
                    }
            ...
            assert!(
                u_run.iter().all(|x| x.is_finite()),
                "Assert 6 FAIL: non-finite at d={d}"
            );
            let max_v = u_run.iter().map(|x| x.abs()).fold(0.0f64, f64::max);
            println!(
                "  d={d}: evolver FINITE (n^d={nd_d}, max|u|={max_v:.3e}), dense expm IMPOSSIBLE"
            );
            let _ = u3;
        }
    ...
    // ── Assert 7a: const-coef reduction (R=0 ⇒ step = k(τ) ≤ 1e-12) ──────
    {
        ...
        assert!(
            max_err < 1e-12,
            "Assert 7a FAIL: const-coef reduction err {max_err:.3e} > 1e-12"
        );
    ...
    // ── Assert 7b: additive a(x) → R equals 0166 per-axis residual ≤ 1e-12 ─
    {
        ...
        let build_lj = |j: usize| -> Vec<f64> {
            let aj: Vec<f64> = xs
                .iter()
                .map(|&x| A0 + 0.2 * (x + 0.4 * j as f64).cos())
                .collect();
            let mut lj = vec![0.0f64; n * n];
            for i in 0..n {
                let ip = (i + 1) % n;
                let im = (i + n - 1) % n;
                let ahp = 0.5 * (aj[i] + aj[ip]);
                let ahm = 0.5 * (aj[i] + aj[im]);
                lj[i * n + ip] += ahp / dx.powi(2);
                lj[i * n + i] -= (ahp + ahm) / dx.powi(2);
                lj[i * n + im] += ahm / dx.powi(2);
            }
            lj
        };
        let lj0 = build_lj(0);
        let lj1 = build_lj(1);
        let l_full: Vec<f64> = (0..nd * nd)
            .map(|i| lift_axis(&lj0, n, d, 0)[i] + lift_axis(&lj1, n, d, 1)[i])
            .collect();
        let lap = lap_1d(n, dx);
        let lx = lift_axis(&lap, n, d, 0);
        let ly = lift_axis(&lap, n, d, 1);
        let r_new: Vec<f64> = (0..nd * nd)
            .map(|i| l_full[i] - A0 * lx[i] - A0 * ly[i])
            .collect();
        ...
        let r_0166: Vec<f64> = (0..nd * nd)
            .map(|i| lift_axis(&r0, n, d, 0)[i] + lift_axis(&r1, n, d, 1)[i])
            .collect();
        ...
        assert!(
            diff_max < 1e-12,
            "Assert 7b FAIL: additive R reduction err {diff_max:.3e} > 1e-12"
        );
