...
use semiflow::{graph_expmv_krylov, scratch::ScratchPool, KrylovPath, SymmetricLinearOp,
               SymmetricOperator};
...
fn build_neumann_csr() -> (Vec<usize>, Vec<u32>, Vec<f64>) {
    ...
    for i in 0..N {
        ...
        if i > 0     { col_idx.push((i - 1) as u32); vals.push(-SCALE); }
        col_idx.push(i as u32); vals.push(diag);
        if i + 1 < N { col_idx.push((i + 1) as u32); vals.push(-SCALE); }
...
impl<'a> CountedOp<'a> {
    ...
    fn total(&self) -> usize { self.count.load(Ordering::Relaxed) }
}

impl SymmetricLinearOp<f64> for CountedOp<'_> {
    fn n(&self) -> usize { self.inner.n() }
    fn lambda_max_bound(&self) -> f64 { self.inner.lambda_max_bound() }
...
fn assert_non_vacuous(
    ...
    assert!(
        ...
        "non-vacuity: λ_max_bound={} < SCALE={SCALE:.0e}", op.lambda_max_bound()
    );
    ...
    assert!(lam1 > 100.0, "non-vacuity: λ_1≈{lam1:.1} < 100 — not stiff enough");
    ...
    let oracle_inf = oracle.iter().cloned().fold(0.0_f64, f64::max);
    assert!(oracle_inf > 0.1, "non-vacuity: oracle ≈ 0 (value={oracle_inf:.3e})");
    (lam1, oracle_inf)
}
...
fn g_symop_implicit_stiff() {
    ...
    let op = SymmetricOperator::from_csr(N, &row_ptr, &col_idx, &vals, 1e-6)
        .expect("Neumann CSR build");
    ...
    let sup_error: f64 = exact.iter().zip(out.iter())
    ...
    assert!(total_mv < explicit_mv_budget,
    ...
    assert!(
        ...
        "G_SYMOP_IMPLICIT_STIFF (c′): implicit not dramatically cheaper — \
         ...
         ratio={:.1}× (need ≥ 100×)", explicit_mv_budget as f64 / total_mv as f64
    );
