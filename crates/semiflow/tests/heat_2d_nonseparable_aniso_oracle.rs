//! Oracle test: anisotropic non-separable 2D heat equation with rotated Gaussian.
//!
//! math.md §10.7-ter.5 + §10.7-ter.7; ADR-0023; gates `T9N_oracle`
//! (constant-β specialisation).
//!
//! PDE: ∂_t u = `axx`·∂_xx u + `ayy`·∂_yy u + `β`·∂_xy u
//!
//! Exact solution via Gaussian propagation (eq. 10.7-ter.13):
//!   `u_0(x,y)` = exp(-x² - y²)  (`Σ_0` = (1/2)·I)
//!
//! `Σ_t` = `Σ_0` + 2t·D, where D = [[axx, β/2], [β/2, ayy]]
//!
//! u(t,x,y) = sqrt(det `Σ_0` / det `Σ_t`) · exp(-½ · [x,y]·`Σ_t`⁻¹·[x,y])
//!
//! This oracle is exact when `β` is a CONSTANT (not position-dependent).
//! The test uses constant `β = 0.05`; the slope test (`strang_nonseparable_aniso_slope.rs`)
//! handles the non-constant β(x,y) = 0.05·exp(-(x²+y²)/4) case via self-convergence.
//!
...
// ---------------------------------------------------------------------------
// Exact solution helpers (eq. 10.7-ter.13)
// ---------------------------------------------------------------------------

/// PDE parameters for constant-coefficient anisotropic 2D heat equation.
struct Params {
    axx: f64,
    ayy: f64,
    beta: f64,
...
/// Exact u(t, x, y) via 2×2 Gaussian propagation (eq. 10.7-ter.13).
...
// ---------------------------------------------------------------------------
// Test-specific function pointers (constant-coefficient PDE, β = 0.05)
// ---------------------------------------------------------------------------
...
/// Constant β = 0.05 — exact oracle applies (eq. 10.7-ter.13).
fn beta_fn(_: f64, _: f64) -> f64 {
    0.05
}
...
/// `T9N_oracle`: constant-β specialisation of eq. (10.7-ter.13).
// n_steps is grid size ≤ 2^16, within f64 mantissa precision.
#[allow(clippy::cast_precision_loss)]
#[test]
fn oracle_constant_beta() {
    let p = Params {
        axx: 0.1,
        ayy: 0.1,
        beta: 0.05,
    ...
    // Anisotropic mixed coupling (constant β = 0.05, beta_norm_bound = 0.05).
    let op = NonSeparable2DAnisotropicChernoff::new(ix, iy, beta_fn, p.beta.abs(), grid).unwrap();
