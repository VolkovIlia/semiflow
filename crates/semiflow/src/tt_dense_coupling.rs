...
pub(crate) fn rank1_dense_matrix<F: SemiflowFloat>(a: &[F], g: &[F], lambda: F) -> Vec<F> {
    let d = a.len();
...
fn compute_sym_im<F: SemiflowFloat>(modes: &[usize], d: usize, sym_d1r: &[F], b: &[F]) -> F {
    let mut im = F::zero();
    for j in 0..d {
...
#[cfg(test)]
mod tests {
    include!("tt_dense_coupling_tests_mod.rs");
}
