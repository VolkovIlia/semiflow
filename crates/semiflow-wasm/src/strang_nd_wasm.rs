...
fn extract_pos_coeff(a: &Float64Array, n: usize, name: &str) -> Result<Vec<f64>, JsValue> {
    ...
    for &v in &buf {
        if !v.is_finite() || v <= 0.0 {
            return Err(make_js_error("NanInf", &format!("{name} must be > 0 and finite")));
        }
    }
    Ok(buf)
}
...
fn var_diff(a_vals: Vec<f64>, amin: f64, amax: f64, n: usize, grid: Grid1D<f64>) -> DiffusionChernoff<f64> {
...
fn evolve_2d(strang: &Strang2Dc, grid: Grid2D<f64>, input: Vec<f64>, tau: f64, n_steps: usize) -> Result<Vec<f64>, JsValue> {
    ...
    for _ in 0..n_steps {
        strang.apply_into(tau, &state, &mut dst, &mut scratch).map_err(|e| err_to_js(&e))?;
        core::mem::swap(&mut state, &mut dst);
    }
    Ok(state.values)
}

fn evolve_3d(strang: &Strang3Dc, grid: Grid3D<f64>, input: Vec<f64>, tau: f64, n_steps: usize) -> Result<Vec<f64>, JsValue> {
...
impl Heat2D {
    ...
    pub fn new(xmin: f64, xmax: f64, nx: usize, ymin: f64, ymax: f64, ny: usize) -> Result<Heat2D, JsValue> {
...
impl Heat3D {
    ...
    pub fn new(
        ...
        Ok(Heat3D { strang, grid, nx, ny, nz })
    }
...
impl Heat2DVarA {
    ...
    pub fn new(
        ...
        Ok(Heat2DVarA { strang, grid, nx, ny })
    }
...
impl Heat3DVarA {
    ...
    pub fn new(
        ...
        Ok(Heat3DVarA { strang, grid, nx, ny, nz })
    }
