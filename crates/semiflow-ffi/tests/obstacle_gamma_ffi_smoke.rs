...
use semiflow_ffi::{
    smf_free_buf_f64, smf_free_buf_u8, smf_obstacle_gamma_free, smf_obstacle_gamma_inactive_gamma,
...
fn obstacle_gamma_new_const_null_out_returns_null_ptr() {
    let s = unsafe { smf_obstacle_gamma_new_const(0.0, 1.0, 8, 0.0, std::ptr::null_mut()) };
...
fn obstacle_gamma_new_array_null_obs_returns_null_ptr() {
    ...
    let s = unsafe { smf_obstacle_gamma_new_array(0.0, 1.0, 8, std::ptr::null(), 8, &mut ptr) };
    assert_eq!(s, SemiflowStatus::NullPtr);
    assert!(ptr.is_null());
}
...
fn obstacle_gamma_new_array_success() {
    ...
    let mut ptr: *mut SmfObstacleGamma = std::ptr::null_mut();
    let s = unsafe { smf_obstacle_gamma_new_array(0.0, 1.0, 8, obs.as_ptr(), 8, &mut ptr) };
    assert_eq!(s, SemiflowStatus::Ok);
    assert!(!ptr.is_null());
    assert_eq!(unsafe { smf_obstacle_gamma_size(ptr) }, 8);
    unsafe { smf_obstacle_gamma_free(ptr) };
...
fn obstacle_gamma_inactive_gamma_constant_v() {
    ...
    for (i, &def) in defined_slice[1..(n - 1)].iter().enumerate() {
        assert_eq!(def, 1, "interior node {i} must be defined");
    }
    // count == n-2 (all interior defined, none excluded by guard band since all inactive).
    assert_eq!(
        count,
        n - 2,
        "count must equal n-2 for constant v above obstacle"
    );
...
fn obstacle_gamma_inactive_gamma_null_returns_null_ptr() {
    let vals = [1.0_f64; 8];
    let mut gamma_out: *mut f64 = std::ptr::null_mut();
    let mut defined_out: *mut u8 = std::ptr::null_mut();
...
fn obstacle_gamma_inactive_gamma_length_mismatch_returns_grid_mismatch() {
    ...
    let vals = [1.0_f64; 5]; // wrong length
    let mut gamma_out: *mut f64 = std::ptr::null_mut();
    let mut defined_out: *mut u8 = std::ptr::null_mut();
    let mut count_out: usize = 0;
    let status = unsafe {
...
fn obstacle_nd2_new_null_out_returns_null_ptr() {
    let s = unsafe { smf_obstacle_nd2_new(0.0, 1.0, 5, 0.0, 1.0, 5, 0.0, std::ptr::null_mut()) };
    assert_eq!(s, SemiflowStatus::NullPtr);
}
...
unsafe fn make_nd2(nx: usize, ny: usize, level: f64) -> *mut SmfObstacleND2 {
    ...
    let s = unsafe { smf_obstacle_nd2_new(0.0, 1.0, nx, 0.0, 1.0, ny, level, &mut ptr) };
...
fn obstacle_nd2_apply_constant_field() {
    ...
    let s = unsafe { smf_obstacle_nd2_apply(ptr, 0.01, v.as_ptr(), n, out.as_mut_ptr(), n) };
...
fn obstacle_nd2_apply_tau_invalid() {
    ...
    let s = unsafe { smf_obstacle_nd2_apply(ptr, 0.0, v.as_ptr(), 25, out.as_mut_ptr(), 25) };
...
fn obstacle_nd2_apply_length_mismatch() {
    ...
    let s = unsafe { smf_obstacle_nd2_apply(ptr, 0.01, v.as_ptr(), 10, out.as_mut_ptr(), 10) };
