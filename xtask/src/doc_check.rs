...
//!
//! Catches three categories of drift before publish:
//!
...
//!    - 1b. Internal-scheme version tokens `vN.M.K` with N≥1 leaked into user-facing READMEs
//!          (escape: `<!-- doc-check: allow-vref -->` on that line).
//!
...
//!
//! ## Fragility ledger (documented limits per design §5)
//!
//! - Check 1a: unconditional denylist; `allow-unpublished` inline marker as escape hatch.
//! - Check 1b: may match legitimate milestone labels like `v1.0.0`; `allow-vref` escapes.
//! - Check 2: `#[pyclass(name=…)]` grep is the canonical ground truth (design §2 critical nuance).
//! - Check 2c: tense heuristic excludes "Rust-only at 0.x" historical qualifiers; a lint, not a prover.
...
/// Scan one line for a `vN.M.K` token where N≥1.
...
fn find_internal_version_token(line: &str) -> Option<String> {
    ...
    while i < bytes.len() {
        ...
        if let Some((tok, end)) = parse_version_token(&bytes[i + 1..]) {
            // N must be ≥ 1 (first digit group non-zero).
            if tok.starts_with('v') {
                // Already ensured by parse; check first digit segment.
                let major_str: String = tok.chars().skip(1).take_while(|c| c.is_ascii_digit()).collect();
                if major_str.parse::<u64>().unwrap_or(0) >= 1 {
                    // Word boundary after token (end = position after last digit).
                    let abs_end = i + 1 + end;
...
/// appears within 40 chars after the denial phrase (the "Rust-only at 0.9.0-beta" exception).
fn check_2c(readme_src: &str, pyclass: &HashSet<String>, violations: &mut Vec<Violation>) {
    // Denial phrases (case-insensitive).
    ...
    // Past-tense qualifiers that exempt the match.
