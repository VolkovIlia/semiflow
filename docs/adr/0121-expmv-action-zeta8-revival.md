...
1. **New module** `crates/semiflow-core/src/expmv.rs` (≤ 500 LoC), `no_std + alloc`, zero new dep:
   ...
   - `θ_m` table as `const THETA_M: [(u32, f64); _]` (Al-Mohy–Higham 2011 Table 3.1, double-precision subset — the values in the PRE-FLIGHT harness `THETA_M_DOUBLE`). **Degree cap `M_MAX = 18`** (SUPERSEDED by ADR-0198: raised to 30, and the θ_m values this line cites were mis-transcribed) (PRE-FLIGHT (a): monomial Horner above arg≈9 loses precision; cap the degree and push remaining argument into `s` — Al-Mohy–Higham Code Fragment 3.1 guard).
